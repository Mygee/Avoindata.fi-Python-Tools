...
class PRHData:
    ...
    def get_prh_data(self, base_directory="", year=2018, start_from_beginning=False):
        ...
        while year <= datetime.datetime.now().year:
            ...
            with open(os.path.join(base_directory, 'data', 'json', 'prh_data', str(year), "failed_requests"),
                      'w') as outfile:
                json.dump(failed_requests, outfile)
            with open(os.path.join(base_directory, 'data', 'json', 'prh_data', str(year), "missing_details_uris"),
                      'w') as outfile:
